...
| `Shift+Tab` or `Alt+M`                             | Toggle permission modes            | Switch between Auto-Accept Mode, Plan Mode, and normal mode. When an agent team is active, also includes Delegate Mode. |
...
| `/model`                  | Select or change the AI model. With Opus 4.6, use left/right arrows to adjust effort level. The change takes effect immediately |
